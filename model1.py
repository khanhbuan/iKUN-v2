...
class Model(nn.Module):
    ...
    def forward(self, x, epoch=1e5):
        ...
        b, t = x['local_img'].shape[:2]
        
        person_indices = (x['bbox'][:, :, 4] == 1.0).nonzero(as_tuple=False)
        car_indices = (x['bbox'][:, :, 4] == 0.0).nonzero(as_tuple=False)
        ...
        if self.opt.kum_mode and (epoch >= self.opt.tg_epoch):
            fused_feat = self.visual_fuse(
                embed, b, t, textual_hidden, self.opt.kum_mode
            )
        else:
            fused_feat = self.visual_fuse(embed, b, t)
    ...
    def visual_fuse(self, embed, b, t, text_feat=None, kum_mode=None):
