...
from FairMOT import get_pose_net, AttentionPool2d
from cmc import apply_affine_to_bboxes, CMCComputer
...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.attnpool = AttentionPool2d(spacial_dim=56, embed_dim=128, num_heads=4, output_dim=1024)

        self.cmc_computer = CMCComputer(method="sparse")

        self.create_pos = nn.Sequential(
            MLP(self.dim*2, self.dim, self.dim, 2),
            nn.Linear(self.dim, self.dim),
        )

        if self.fuse_mode == "cat":
            self.fuse = MLP(self.dim*3, self.dim, self.dim, 2)
      
        if self.fuse_mode == "attn":
            self.fuse1 = nn.MultiheadAttention(
                            embed_dim=self.dim,
                            num_heads=4, 
                            dropout=0.,
                        )
            self.fuse2 = nn.MultiheadAttention(
                            embed_dim=self.dim,
                            num_heads=4, 
                            dropout=0.,
                        )
            self.fuse3 = nn.MultiheadAttention(
                            embed_dim=self.dim,
                            num_heads=4, 
                            dropout=0.,
                        )
    ...
    def forward(self, x, epoch=1e5):
        ...
        if self.opt.kum_mode and (epoch >= self.opt.tg_epoch):
            fused_feat = self.visual_fuse(
                x['local_img'], x['full_img'], x['bbox'], textual_hidden
            )
        else:
            fused_feat = self.visual_fuse(x['local_img'], x['global_img'])
        print(fused_feat.shape, textual_feat.shape)
    ...
    def visual_fuse(self, local_img, full_images, bbox, text_feat=None):
        b, t = full_images.size()[:2]
        # spatial encoding
        local_img = rearrange(local_img, 'b t c h w -> (b t) c h w')
        local_feat = self.clip.encode_image(local_img)  # [bt,c]

        """
        sửa lại
        """
        # reid_feat = self.attnpool(self.reid(local_img)[0]['id']) # [bt,c]

        full_images = rearrange(full_images, 'b t c H W -> (b t) c H W')
        bbox = rearrange(bbox, ' b t n -> (b t) n')

        _,_, H, W = full_images.shape
        bbox = bbox * torch.tensor([W, H, W, H])
        affine_bboxes = apply_affine_to_bboxes(self.cmc_computer, full_images, bbox).cuda()
        affine_bboxes = affine_bboxes / (torch.tensor([W, H, W, H]).cuda())
        affine_bboxes = affine_bboxes.reshape(b, t, -1).to(torch.float)
        pos_feat = self.create_pos(gen_sineembed_for_position(affine_bboxes, img_dim=self.dim))

        if self.fuse_mode == "add":
            visual_feat = local_feat + reid_feat + pos_feat
            fused_feat = self.cross_modal_fusion(visual_feat, text_feat, b, t)

        if self.fuse_mode == "cat":
            visual_feat = torch.cat((local_feat, reid_feat, pos_feat), dim=1)
            visual_feat = self.fuse(visual_feat)
            fused_feat = self.cross_modal_fusion(visual_feat, text_feat, b, t)

        if self.fuse_mode == "attn":
            local_feat = self.fuse1(query=local_feat, key=text_feat, value=text_feat)[0]
            reid_feat = self.fuse2(query=reid_feat, key=text_feat, value=text_feat)[0]
            pos_feat = self.fuse3(query=pos_feat, key=text_feat, value=text_feat)[0]

            fused_feat = torch.stack((local_feat, reid_feat, pos_feat), dim=1)
            fused_feat = torch.mean(visual_feat, dim=1)
