...
def get_model_16(opt, name='Model'):
    model = eval(name)(opt)
    model.to("cuda")
...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.combine = MLP(self.dim + self.pos_dim + self.reid_dim, self.dim, self.dim, num_layers=1)
    ...
    def forward(self, x, epoch=1e5):
        ...
        embed = rearrange(embed, 'b t c -> (b t) c')[None,:,:]
    ...
    def visual_fuse(self, local_img, bbox=None, embed=None, text_feat=None, kum_mode=None):
        ...
        q_pos = gen_sineembed_for_position(bbox)
