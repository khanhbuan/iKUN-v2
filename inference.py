...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            bbox = input['bbox']
            coor = bbox[:, bbox.shape[1]-1, :4]
            delta = bbox[:, bbox.shape[1]-1,:4] - bbox[:, bbox.shape[1]-2,:4]
            print(coor.shape, delta.shape, bbox[:, bbox.shape[1]-1, 4].shape)
            new_bbox = torch.cat([coor, delta, bbox[:, bbox.shape[1]-1, 4][:, None]], dim=1)
            print(new_bbox)
            # print(output["logits"])
            break