...
def get_model_26(opt, name='Model'):
    model = eval(name)(opt)
    model.to("cuda")
...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.pos_enc = nn.Sequential(
            MLP(9, self.dim, self.dim, num_layers=2),
    ...
    def cross_modal_fusion(self, vis_feat, text_feat, b, t):
        ...
        text_feat = text_feat.repeat([1, t-1, 1, 1])
    ...
    def visual_fuse(self, bbox, text_feat=None, kum_mode=None):
        ...
        coor1 = bbox[:, bbox.shape[1]-1, :4]
        coor2 = bbox[:, bbox.shape[1]-2, :4]
        center1 = torch.Tensor([coor1[:, 0] + coor1[:, 2] / 2.0, coor1[:, 1] + coor1[:, 3] / 2.0])
        center2 = torch.Tensor([coor2[:, 0] + coor2[:, 2] / 2.0, coor2[:, 1] + coor2[:, 3] / 2.0])
        
        delta = center1 - center2
        bbox = torch.cat([coor1, delta, bbox[:, bbox.shape[1]-1, 4][:, None]], dim=1)[:, None, :]
