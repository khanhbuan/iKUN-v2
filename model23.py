...
def get_model_23(opt, name='Model'):
    model = eval(name)(opt)
    model.to("cuda")
...
def gen_sineembed_for_position(pos_tensor, img_dim=1024):
    ...
    wh_embed = (pos_tensor[:, :, 2] * pos_tensor[:, :, 3]) * scale
    pos_wh = wh_embed[:, :, None] / dim_t
    pos_wh = torch.stack((pos_wh[:, :, 0::2].sin(), pos_wh[:, :, 1::2].cos()), dim=3).flatten(2)

    pos = torch.cat((pos_x1, pos_y1, pos_x2, pos_y2, pos_w, pos_h, pos_wh), dim=2)
    return pos
...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.pos_enc = nn.Sequential(
            MLP(self.dim//2*7, self.dim, self.dim, num_layers=2),
            # MLP(7, self.dim, self.dim, num_layers=2),
    ...
    def visual_fuse(self, bbox, text_feat=None, kum_mode=None):
        ...
        # x1, y1, x2, y2, w, h = bbox[:,:,0][:,:,None], bbox[:,:,1][:,:,None], (bbox[:,:,2]+bbox[:,:,0])[:,:,None], (bbox[:,:,3]+bbox[:,:,1])[:,:,None], bbox[:,:,2][:,:,None], bbox[:,:,3][:,:,None]
        # bbox=torch.cat([x1, y1, x2, y2, w, h, w*h], dim=-1)
        ...
        if kum_mode is not None:
            ...
            fused_feat = rearrange(pos_feat, 'l bt c -> bt c l') # l = 1
